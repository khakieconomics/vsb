...
merger_results_path       =[code_path,'\merger results full\'];
...
if optrout~=6
    ...
    if optrout<=9
        outfile    =[merger_results_path,'nevo_merger_log_0',num2str(optrout),'.txt']; 
        merger_file=[merger_results_path,'nevo_merger_results_0',num2str(optrout),'.txt']; 
        fsolve_file=[merger_results_path,'nevo_fsolve_results_0',num2str(optrout),'.txt']; 
    else
       outfile    =[merger_results_path,'nevo_merger_log_',num2str(optrout),'.txt'];  
       merger_file=[merger_results_path,'nevo_merger_results_',num2str(optrout),'.txt']; 
       fsolve_file=[merger_results_path,'nevo_fsolve_results_',num2str(optrout),'.txt'];                  
    end
    ...
    load (matfile, 'perturbs2', 'fvals', 'theta1s', 'theta2s','exit_infos', 'hessians','hessians2',...
        'gradients', 'gradients2' ,'gradients3','deltas' ,'gmmresids' ,'mvalolds2','std_errors','counts2');
...
%         gmmresid = deltajt - x1*theta1;
        ...
        %product market shares pre-merger
        ...
        exitflags=zeros(nmkt,1);
        fvals=zeros(nmkt,nbrn);

        disp('Solve for the new equilibrium prices');       
        global cdid beta ns mc deltajt0 market_no nmkt...
            nbrn x2 own_dummy x1 rcoeffs rcoeffs2
        
        for market_no=1:94
            tic
            fprintf('=================================\n');
            fprintf('Solving market: %5i\n',market_no);
            fprintf('=================================\n');
            options=optimset('Display','iter',...
                'TolFun',0.01,...
                'TolX',  0.001);
            deltajt0=deltajt-price_pre*theta1(1);
            [price_post(cdid==market_no,:),fval,exitflag,output,jacobians]=fsolve('merger_focs',price_approx(cdid==market_no),options);
            exitflags(market_no,1)=exitflag;
            fvals(market_no,:)=fval';
            toc
        end
        ...
        fprintf('routine   : %5i\t',optrout);
        fprintf('start val : %5i\n',perturb);

        merger_results=[merger_results;[optrout_aux,jj_merger_aux,market,brand,price_pre,price_post,sjt_pre,sjt_post,mc,profit_pre,profit_post,mean_CV_aux,CV_pre_aux,CV_post_aux]];
        fsolve_results=[exitflags,fvals];
    end

   cd(merger_results_path)
   save(merger_file,'merger_results','-ASCII');
   save(fsolve_file,'fsolve_results','-ASCII');
